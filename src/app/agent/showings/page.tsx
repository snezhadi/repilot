...
export default function AgentShowingsPage() {
  ...
  const filteredProperties = useMemo(() => {
    const byStatus =
      filterStatus === "all"
        ? agentProperties
        : agentProperties.filter((property) => property.showing.status === filterStatus);

    if (clientFilter === "all-clients") {
      return byStatus;
    }

    return byStatus.filter((property) => property.showing.clientId === clientFilter);
  }, [filterStatus, clientFilter]);

  const clientColors = useMemo(() => {
    return filteredProperties.reduce<Record<string, string>>((acc, property, index) => {
      const clientId = property.showing.clientId;
      if (!clientId || acc[clientId]) return acc;
      const palette = [
      ...
      acc[clientId] = palette[index % palette.length];
      return acc;
    }, {});
  }, [filteredProperties]);
  ...
  return (
    <div className="min-h-screen bg-background">
      <CustomSidebar activePage="showings" mode="agent" />

      <div className="ml-16 p-6">
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-foreground mb-2">Client Showings</h1>
          <p className="text-muted-foreground">
            Track upcoming and completed showings across all of your active clients.
          </p>
        </div>
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="relative">
                <select
                  ...
                  className="appearance-none pr-8 pl-3 py-2 border border-input rounded-md bg-background text-sm focus:outline-none focus:ring-2 focus:ring-ring"
                >
                  <option value="all-clients">All clients</option>
                  {INITIAL_AGENT_CLIENTS.filter((client) => client.status === "active").map((client) => (
            ...
            <p className="text-muted-foreground">
              {filterStatus === "all"
                ? clientFilter === "all-clients"
                  ...
                  : "No showings scheduled for this client."
                  ...
                  <Card key={property.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                    ...
                    <CardContent className="p-4 space-y-3">
                      <div className="flex items-start justify-between">
                        ...
                        {property.showing.clientId && (
                          <Badge variant="secondary" className="bg-muted text-muted-foreground">
                            {
                              INITIAL_AGENT_CLIENTS.find((client) => client.id === property.showing.clientId)?.name ||
                              "Client"
                            }
                          </Badge>
                        )}
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <Button size="sm" className="text-xs" onClick={() => handlePropertyClick(property.id)}>
                        ...
                        <tr key={property.id} className="border-t border-border hover:bg-muted/30">
                          <td className="p-4">
                            <div className="flex items-center space-x-3">
                              <img src={property.image} alt={property.title} className="w-16 h-12 object-cover rounded" />
                              <span className="font-medium text-sm line-clamp-2">{property.title}</span>
                            </div>
                          </td>
                          ...
                          <td className="p-4 text-sm text-muted-foreground flex items-center">
                            <MapPin className="w-4 h-4 mr-1" />
                            {property.location}
                          </td>
                          <td className="p-4" onClick={() => handleStatusClick(property)}>
                            {getStatusBadge(
                              property.showing.status,
                              property.showing.scheduledDate,
                              property.showing.scheduledTime
                  ...
                  <div className="absolute bottom-4 left-4 right-4 z-10">
                    <Card className="overflow-hidden shadow-2xl">
                      ...
                      <div className="flex p-4 gap-4">
                        <img src={selectedMapProperty.image} alt={selectedMapProperty.title} className="w-32 h-24 object-cover rounded-lg" />
                        <div className="flex-1">
                          <div className="flex items-start justify-between mb-2">
                            <div>
                              <h3 className="font-semibold text-lg mb-1 line-clamp-1">{selectedMapProperty.title}</h3>
                              <p className="text-xl font-bold text-primary">{selectedMapProperty.price}</p>
                            </div>
                            {selectedMapProperty.showing.clientId && (
                              <Badge variant="secondary" className="bg-muted text-muted-foreground">
                                {
                                  INITIAL_AGENT_CLIENTS.find((client) => client.id === selectedMapProperty.showing.clientId)?.name ||
                                  "Client"
                                }
                              </Badge>
                            )}
                          ...
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items:center space-x-2">
                              ...
                              <Button size="sm" className="text-xs" onClick={() => handlePropertyClick(selectedMapProperty.id)}>
                ...
                <div className="grid grid-cols-7 gap-2">
                  {Array.from({ length: 35 }).map((_, index) => {
                    ...
                    return (
                      ...
                      >
                        <div className="text-sm font-medium mb-2">{isCurrentMonth ? day : ""}</div>
                        <div className="space-y-1">
                          {dayShowings.map((property) => {
                            const clientId = property.showing.clientId;
                            const colorClass = clientId
                              ? clientColors[clientId] || "bg-blue-200 text-blue-900"
                              : "bg-blue-200 text-blue-900";
                            const clientName = clientId
                              ? INITIAL_AGENT_CLIENTS.find((client) => client.id === clientId)?.name
                              : undefined;
                            ...
                            return (
                              <button
                                key={property.id}
                                onClick={() => setSelectedMapProperty(property)}
                                className={`w-full text-left text-xs p-1.5 rounded hover:opacity-80 transition-opacity line-clamp-2 ${colorClass}`}
                                title={`${property.title} - ${property.showing.scheduledDate} at ${property.showing.scheduledTime || "TBD"}`}
                              >
                                <div className="font-semibold">
                                  {property.showing.scheduledTime || "TBD"}
                                  {clientName ? ` • ${clientName}` : ""}
                                </div>
                                <div>{property.title.substring(0, 24)}...</div>
                              </button>
                            );
                          })}
                        </div>
